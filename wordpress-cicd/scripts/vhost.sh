...
/usr/bin/xmlstarlet $ED_CMD -s "httpServerConfig/virtualHostList/virtualHost[name = "$VHOST_TARGET"]" -t elem -n "vhRoot" -v "\$SERVER_ROOT/webroot/" ${HTTPD_CONFIG} 2>&1;
/usr/bin/xmlstarlet $ED_CMD -s "httpServerConfig/virtualHostList/virtualHost[name = "$VHOST_TARGET"]" -t elem -n "configFile" -v "\$VH_ROOT/$VHOST_TARGET.xml" ${HTTPD_CONFIG} 2>&1;
/usr/bin/xmlstarlet $ED_CMD -s "httpServerConfig/virtualHostList/virtualHost[name = "$VHOST_TARGET"]" -t elem -n "allowSymbolLink" -v "1" ${HTTPD_CONFIG} 2>&1;
/usr/bin/xmlstarlet $ED_CMD -s "httpServerConfig/virtualHostList/virtualHost[name = "$VHOST_TARGET"]" -t elem -n "enableScript" -v "1" ${HTTPD_CONFIG} 2>&1;
/usr/bin/xmlstarlet $ED_CMD -s "httpServerConfig/virtualHostList/virtualHost[name = "$VHOST_TARGET"]" -t elem -n "restrained" -v "1" ${HTTPD_CONFIG} 2>&1;
/usr/bin/xmlstarlet $ED_CMD -s "httpServerConfig/virtualHostList/virtualHost[name = "$VHOST_TARGET"]" -t elem -n "setUIDMode" -v "0" ${HTTPD_CONFIG} 2>&1;
/usr/bin/xmlstarlet $ED_CMD -s "httpServerConfig/virtualHostList/virtualHost[name = "$VHOST_TARGET"]" -t elem -n "chrootMode" -v "0" ${HTTPD_CONFIG} 2>&1;
